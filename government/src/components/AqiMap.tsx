...
export function AqiMap({ city }: { city: string }) {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (points.length === 0) {
        ...
        const src = [
          openaqReadings.length > 0 ? `${openaqReadings.length} air monitors` : '',
          overpassPlaces.length > 0 ? `${overpassPlaces.length} local hubs`      : '',
        ].filter(Boolean).join(' + ');
        setDataSource(src);
      }
    ...
    return (
      <Card className="h-[450px] flex items-center justify-center">
        <div className="text-center space-y-2">
          ...
          <p className="text-sm text-muted-foreground">Initializing ground network…</p>
        </div>
      </Card>
    );
  }
  ...
  return (
    <Card className="overflow-hidden border-2 border-primary/10 shadow-lg">
      <CardHeader className="bg-primary/5 border-b py-3 px-4">
        <CardTitle className="flex items-center justify-between flex-wrap gap-2 text-primary">
          <div className="flex items-center gap-2 font-bold text-sm uppercase tracking-wider">
            <MapPin className="h-4 w-4" /> Ground Observation Map
          </div>
          <div className="flex items-center gap-2">
            <Badge variant="secondary" className="font-mono text-[10px] uppercase tracking-widest bg-primary/10 text-primary border-primary/20">
              {city}
            </Badge>
          </div>
        </CardTitle>
      ...
      <CardContent className="p-0">
        ...
        <div className="relative h-[380px] w-full grayscale-[0.2] hover:grayscale-0 transition-all duration-500">
          <MapContainer
                  ...
                  <Popup>
                    <div className="p-1 min-w-[150px]">
                      <div className="flex justify-between items-center mb-1">
                        <strong className="text-sm font-bold">{h.label}</strong>
                        <span className="text-[9px] opacity-60 font-semibold px-1 rounded bg-muted">
                          {h.source.toUpperCase()}
                        </span>
                      </div>
                      <div className="text-[11px] text-muted-foreground mb-1">{TYPE_LABELS[h.type] ?? h.type}</div>
                      <div className="flex items-center gap-2">
                        <div className="text-[11px] font-bold">AQI: {h.aqi}</div>
                        {h.pm25 != null && <div className="text-[10px] opacity-60">PM2.5: {h.pm25.toFixed(1)}</div>}
                      </div>
                      <div className="mt-2 text-[10px] text-white rounded py-1 px-2 text-center font-bold"
          ...
          <div className="absolute bottom-4 left-4 z-[1000] bg-background/90 backdrop-blur-md rounded-xl border-2 border-primary/20 p-4 shadow-xl">
            <p className="text-[10px] font-bold uppercase tracking-wider text-primary mb-3">Pollution Scale</p>
            <div className="space-y-1.5">
              {[
                ...
                <div key={l.label} className="flex items-center gap-3">
                  <span className="h-2.5 w-2.5 rounded-full flex-shrink-0 animate-pulse" style={{ background: l.color, boxShadow: `0 0 5px ${l.color}` }} />
                  <span className="text-[10px] font-medium text-muted-foreground">{l.label}</span>
                  <span className="text-[10px] text-muted-foreground/50 ml-auto font-mono">{l.range}</span>
                </div>
              ))}
            </div>
          </div>
