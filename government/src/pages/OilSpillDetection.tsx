...
const SEVERITY_COLOR: Record<string, string> = {
  Major: "#dc2626", Moderate: "#f97316", Minor: "#facc15",
};
...
function BBoxDrawer({ onBBoxChange, drawing }: { onBBoxChange: (bbox: BBox | null) => void; drawing: boolean }) {
  const map = useMap();
  ...
  useMapEvents({
    mousedown(e) { if (!drawing) return; startRef.current = e.latlng; onBBoxChange(null); setCurrent(null); },
    mousemove(e) {
      if (!drawing || !startRef.current) return;
      const s = startRef.current;
      setCurrent({ minLat: Math.min(s.lat, e.latlng.lat), maxLat: Math.max(s.lat, e.latlng.lat), minLon: Math.min(s.lng, e.latlng.lng), maxLon: Math.max(s.lng, e.latlng.lng) });
    },
    mouseup(e) {
      if (!drawing || !startRef.current) return;
      const s = startRef.current;
      onBBoxChange({ minLat: Math.min(s.lat, e.latlng.lat), maxLat: Math.max(s.lat, e.latlng.lat), minLon: Math.min(s.lng, e.latlng.lng), maxLon: Math.max(s.lng, e.latlng.lng) });
      startRef.current = null; setCurrent(null);
    },
  });

  if (!current) return null;
  return <Rectangle bounds={[[current.minLat, current.minLon], [current.maxLat, current.maxLon]]} pathOptions={{ color: "#3b82f6", fillColor: "#3b82f6", fillOpacity: 0.15, weight: 2, dashArray: "6 4" }} />;
}

function CursorStyle({ drawing }: { drawing: boolean }) {
  const map = useMap();
  useEffect(() => { map.getContainer().style.cursor = drawing ? "crosshair" : ""; }, [drawing, map]);
  return null;
}
...
export default function OilSpillDetection() {
  ...
  const handleBBoxChange = useCallback((b: BBox | null) => {
    setBbox(b); setSpills(null); setError(null);
  ...
  const handleDetect = async () => {
    ...
    if (w > 5 || h > 5) { setError("Area too large — keep the box under 5° × 5°."); return; }
    setLoading(true); setError(null); setSpills(null);
    try {
      const res = await fetchOilSpills((bbox.minLat + bbox.maxLat) / 2, (bbox.minLon + bbox.maxLon) / 2, Math.max(w, h) / 2);
      setSpills(res.spills);
      setDetectionImage(res.detection_image ?? null);
      setScanDate(new Date(res.generated_at).toLocaleString());
    } catch (e: any) { setError(e.message ?? "Detection failed"); }
    finally { setLoading(false); }
  };

  const handleReset = () => { setBbox(null); setSpills(null); setDetectionImage(null); setError(null); setDrawing(false); };
      ...
      <div className="flex items-start justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-xl font-bold flex items-center gap-2">
            <Droplets className="h-5 w-5 text-orange-500" /> Oil Spill Detection
          </h1>
          <p className="text-sm text-muted-foreground mt-1">Draw a bounding box on the map to scan for oil spills using Sentinel-1 SAR satellite imagery.</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-3">
          <div className="flex items-center gap-2 flex-wrap">
            <Button size="sm" variant={drawing ? "default" : "outline"}
            ...
            <Button size="sm" disabled={!bbox || loading} onClick={handleDetect} className="bg-orange-600 hover:bg-orange-700 text-white">
              {loading ? <><Loader2 className="h-3.5 w-3.5 mr-1.5 animate-spin" />Scanning…</> : <><Droplets className="h-3.5 w-3.5 mr-1.5" />Detect Spills</>}
            </Button>
            {(bbox || spills) && <Button size="sm" variant="ghost" onClick={handleReset}><RotateCcw className="h-3.5 w-3.5 mr-1.5" /> Reset</Button>}
            <Button size="sm" variant="outline" onClick={() => setSatellite(s => !s)} className={satellite ? "border-sky-500 text-sky-500 bg-sky-500/10" : ""}>
              {satellite ? <><Map className="h-3.5 w-3.5 mr-1.5" />Street View</> : <><Satellite className="h-3.5 w-3.5 mr-1.5" />Satellite View</>}
            </Button>
            {bbox && !loading && !spills && <span className="text-xs text-muted-foreground">Box: {(bbox.maxLon - bbox.minLon).toFixed(2)}° × {(bbox.maxLat - bbox.minLat).toFixed(2)}°</span>}
          </div>

          <Card className="overflow-hidden">
            <div className="relative h-[480px]">
              <MapContainer center={[20, 78]} zoom={4} style={{ height: "100%", width: "100%" }} scrollWheelZoom>
                ...
                {bbox && <Rectangle bounds={[[bbox.minLat, bbox.minLon], [bbox.maxLat, bbox.maxLon]]} pathOptions={{ color: "#3b82f6", fillColor: "#3b82f6", fillOpacity: 0.1, weight: 2 }} />}
                ...
                {spills?.map((spill, i) => {
                  ...
                  return (
                    <CircleMarker key={i} center={[spill.latitude, spill.longitude]} radius={radius} pathOptions={{ color, fillColor: color, fillOpacity: 0.6, weight: 2 }}>
                      <Popup>
                        <div style={{ minWidth: 170 }}>
                          <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 6 }}>
                            <span>🛢️</span><strong style={{ fontSize: 13 }}>Oil Spill #{i + 1}</strong>
                          </div>
                          ...
                          <div style={{ marginTop: 6, fontSize: 10, background: color, color: "#fff", borderRadius: 4, padding: "2px 8px", textAlign: "center", fontWeight: 700 }}>{spill.severity} Spill</div>
                        </div>
                      </Popup>
                    </CircleMarker>
                  );
          ...
          {/* Annotated SAR detection image */}
          {detectionImage && (
            <Card>
              <CardHeader className="py-3 px-4 border-b">
                <CardTitle className="text-sm flex items-center gap-2">
                  <Satellite className="h-4 w-4 text-sky-500" /> Sentinel-1 SAR Detection Output
                </CardTitle>
              </CardHeader>
              <CardContent className="p-3">
                <img
                  src={`data:image/png;base64,${detectionImage}`}
                  alt="Annotated SAR detection"
                  className="w-full rounded-lg border border-border object-contain max-h-[400px]"
                />
                <p className="text-xs text-muted-foreground mt-2">
                  Colored contours = detected oil spills on Sentinel-1 VV SAR imagery. Red = Major, Orange = Moderate, Cyan = Minor.
                </p>
              </CardContent>
            </Card>
          )}
          ...
          {spills !== null && (
            <>
              ...
              <div className="grid grid-cols-3 gap-2">
                {[{ label: "Major", count: majorCount, cls: SEVERITY_BG.Major }, { label: "Moderate", count: moderateCount, cls: SEVERITY_BG.Moderate }, { label: "Minor", count: minorCount, cls: SEVERITY_BG.Minor }].map(({ label, count, cls }) => (
          ...
          <Card>
            <CardHeader className="py-3 px-4 border-b">
              <CardTitle className="text-sm flex items-center justify-between">
                Detected Spills
                {spills !== null && <Badge variant="outline" className="text-[10px]">{spills.length} found</Badge>}
              </CardTitle>
            </CardHeader>
                ...
                <div className="divide-y max-h-[380px] overflow-y-auto">
                  {spills.map((spill, i) => (
                    <div key={i} className="px-4 py-3 flex items-start gap-3">
                      <div className="mt-0.5 h-3 w-3 rounded-full shrink-0" style={{ background: SEVERITY_COLOR[spill.severity] }} />
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between gap-2">
                          <span className="text-sm font-medium">Spill #{i + 1}</span>
                          <Badge variant="outline" className={`text-[10px] shrink-0 ${SEVERITY_BG[spill.severity]}`}>{spill.severity}</Badge>
                        </div>
                        <p className="text-xs text-muted-foreground mt-0.5">{spill.area_km2.toFixed(3)} km²</p>
                        <p className="text-[10px] text-muted-foreground/60 font-mono mt-0.5">{spill.latitude.toFixed(4)}°, {spill.longitude.toFixed(4)}°</p>
                      </div>
                    </div>
                  ))}
                </div>
